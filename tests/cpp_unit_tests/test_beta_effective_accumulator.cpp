...
TEST_CASE("F-CLUTCH beta_eff accumulator scores spatial I-star")
{
  ...
  acc.begin_batch(1);
  acc.score_fission_site({0.25, 0.25, 0.25}, 1.0, 0, 1.0, 0.0);
  acc.score_fission_site({0.25, 0.25, 0.25}, 2.0, 1, 2.0, 5.0);
  acc.end_batch(1);

  acc.begin_batch(2);
  acc.score_fission_site({1.25, 0.25, 0.25}, 4.0, 2, 3.0, 7.0);
  acc.end_batch(2);
  ...
  REQUIRE_THAT(time.denominator, Catch::Matchers::WithinRel(55.0, 1.0e-12));
  REQUIRE_THAT(
    time.lifetime_numerator, Catch::Matchers::WithinRel(145.0, 1.0e-12));
  REQUIRE_THAT(time.emission_adjusted_lifetime_numerator,
    Catch::Matchers::WithinRel(475.0, 1.0e-12));
  REQUIRE_THAT(time.transport_lifetime,
    Catch::Matchers::WithinRel(145.0 / 55.0, 1.0e-12));
  REQUIRE_THAT(time.emission_adjusted_lifetime,
    Catch::Matchers::WithinRel(475.0 / 55.0, 1.0e-12));
