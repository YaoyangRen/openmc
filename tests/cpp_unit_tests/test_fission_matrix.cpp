...
TEST_CASE("Fission matrix skips early inactive batches")
{
  ...
  FissionMatrix matrix(grid, 5, {}, 3);

  matrix.start_new_batch(1);
  matrix.record_source_birth({0.25, 0.25, 0.25}, 1, -1.0, -1, 1.0);
  matrix.record_fission_site({0.25, 0.25, 0.25}, 1.0, 1, -1.0, -1);

  matrix.start_new_batch(2);
  matrix.record_source_birth({0.25, 0.25, 0.25}, 2, -1.0, -1, 1.0);
  matrix.record_fission_site({0.25, 0.25, 0.25}, 1.0, 2, -1.0, -1);

  matrix.start_new_batch(3);
  matrix.record_source_birth({0.25, 0.25, 0.25}, 3, -1.0, -1, 2.0);
  matrix.record_fission_site({0.25, 0.25, 0.25}, 2.0, 3, -1.0, -1);
